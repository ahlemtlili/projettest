...
import { deleteParent, getAllParents, getCurrentuser } from '../../Redux/actions/userActions';
import { Link } from 'react-router-dom';
import Button from 'react-bootstrap/Button';
import { MdDeleteSweep } from 'react-icons/md';

export default function ParentG() {
    const parents= useSelector(state=>state.userReducer.parents)
      ...
      <Table sx={{ minWidth: 650}} aria-label="caption table">
        ...
        {parents.map((el) => (
            ...
            <TableRow key={el._id}>
              ...
              <TableCell align="left" style={{"fontSize":"20px" }}><div><Button style={{"fontSize":"20px"}} onClick ={()=> dispatch(deleteParent(el._id))} size="small"><MdDeleteSweep/>Delete</Button></div></TableCell>
